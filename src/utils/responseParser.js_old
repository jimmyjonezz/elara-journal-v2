...
const { repair } = require('jsonrepair');  // ← Правильный импорт
...
function extractJSON(rawText) {
  ...
  // 2. 🔥 Ищем первую { и последнюю } (игнорируя внутри строк)
  let firstBrace = -1;
  let lastBrace = -1;
  let inString = false;
  let escaped = false;
  ...
  for (let i = 0; i < text.length; i++) {
    ...
    if (char === '"' && !escaped) {
      ...
      firstBrace = i;    } else if (char === '}' && !inString) {
      lastBrace = i;
    }
...
async function saveFailedParse(rawText, errorMessage) {
  try {
    ...
    const content = [
      `ERROR: ${errorMessage}`,
      `TIMESTAMP: ${new Date().toISOString()}`,      `POSITION: ${position || 'N/A'}`,
      '─'.repeat(80),
...
/**
 * Многоуровневый безопасный парсинг JSON с jsonrepair
 */
function safeParseJSON(rawText, options = {}) {
  ...
  const attempts = [
    ...
    () => JSON.parse(repair(extractedJSON)),   // Попытка 2: ремонт через jsonrepair
  ];
  ...
  for (let i = 0; i < attempts.length; i++) {
    try {
      const result = attempts[i]();      if (result && typeof result === 'object') {
