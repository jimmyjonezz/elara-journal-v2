 ...
 * Исправленная версия — правильная область видимости и логика
 */
function escapeQuotesInValues(text) {
  ...
  for (let i = 0; i < text.length; i++) {
    ...
    // Кавычка
    if (char === '"' && !escaped) {
      // Проверяем, что идёт после кавычки
      const remaining = text.slice(i + 1);      const isClosingQuote = /^\s*[,}\]]/.test(remaining);
...
module.exports = {
  parseCriticResponse,
  safeParseJSON,
  repairAndExtractJSON,  escapeQuotesInValues,
