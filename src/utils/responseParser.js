 ...
 * Экранирует только кавычки ВНУТРИ значений, не трогая структурные
 */
function escapeQuotesInValues(text) {
  ...
  for (let i = 0; i < text.length; i++) {
    ...
    // Отслеживаем обратный слэш    if (char === '\\' && !escaped) {
      escaped = true;
      result += char;
      continue;
    }
    
    // Двоеточие — после него ожидается значение
    if (char === ':' && !inString) {
      afterColon = true;
      result += char;
      escaped = false;
    ...
    if (char === '"') {
      if (!escaped) {
        // Проверяем контекст
        const before = text.slice(Math.max(0, i - 20), i).trimEnd();
        const isKeyStart = /[{,]\s*$/.test(before) || before === '';
        const isValueStart = afterColon && !inString;
        
        if (isKeyStart || isValueStart) {
          // Это структурная кавычка (начало ключа или значения)
          inString = !inString;
          afterColon = false;
        } else if (inString) {
          // Кавычка внутри строки — проверяем, закрывающая ли это
          const after = text.slice(i + 1, i + 5).trimStart();
 ...
 * 🔥 МИНИМАЛИСТИЧНАЯ ВЕРСИЯ — с умным экранированием кавычек
 */
function repairAndExtractJSON(rawText) {
  ...
  // 6. 🔥 Экранируем кавычки внутри значений (только если нужно)
...
async function saveFailedParse(rawText, errorMessage) {
  try {
    ...
    const ts = new Date().toISOString().replace(/[:.]/g, '-');    const filePath = path.join(logsDir, `parse-fail-${ts}.txt`);
