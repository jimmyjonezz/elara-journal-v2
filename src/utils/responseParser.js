...
// 🔥 Правильный импорт для jsonrepair@3.13.3
let repair;
try {
  // Попытка 1: Импортируем как ESM (для новых версий)
  const jsonrepairModule = require('jsonrepair');
  repair = jsonrepairModule.repair || jsonrepairModule.default || jsonrepairModule;
} catch (e) {
  // Попытка 2: Fallback — используем встроенную функцию
...
/**
 * 🔥 Встроенная функция экранирования кавычек (fallback)
 */
...
function extractJSON(rawText) {
  ...
  // 2. 🔥 Умный поиск { и } (игнорируем внутри строк)
  let firstBrace = -1;
  let lastBrace = -1;
  let inString = false;
  let escaped = false;
  ...
  for (let i = 0; i < text.length; i++) {
    ...
    if (char === '"' && !escaped) {
      inString = !inString;    } else if (char === '{' && !inString && firstBrace === -1) {
      firstBrace = i;
...
function safeParseJSON(rawText, options = {}) {
  ...
  const attempts = [
    ...
    () => {
      // Попытка 2: ремонт через jsonrepair
      const repaired = repair(extractedJSON);
