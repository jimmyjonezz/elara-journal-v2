...
/**
 * 🔥 АГРЕССИВНАЯ очистка невидимых символов
 * Вызывается ПЕРВОЙ, до любых других операций
 */
...
function repairAndExtractJSON(rawText) {
  ...
  // 🔥 1. ПЕРВОЕ: Удаляем невидимые символы
  ...
  // 🔥 3. ВНОВЬ удаляем невидимые символы (могли остаться после markdown)
  text = removeInvisibleChars(text);
  ...
  // 🔥 4. ЕЩЁ РАЗ удаляем невидимые символы (после вырезки)
  text = removeInvisibleChars(text);

  // 5. Удаляем trailing commas перед } или ]
  text = text.replace(/,\s*([}\]])/g, '$1');

  // 6. Экранируем кавычки внутри строк
  text = escapeQuotesInStringValues(text);

  // 7. Заменяем «ёлочки» на обычные кавычки
  text = text.replace(/«/g, '"').replace(/»/g, '"');

  // 8. Экранируем переносы строк внутри строковых значений
  text = text.replace(/(?<!\\)\n/g, '\\n');
...
async function saveFailedParse(rawText, errorMessage) {
  try {
    ...
    // Показываем HEX-коды символов вокруг ошибки
    let hexContext = 'N/A';
...
function parseCriticResponse(rawResponse) {
  ...
  try {
    ...
    console.error('💥 Ошибка парсинга ответа критика:', e.message);    return {
