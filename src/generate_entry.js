...
async function prepareEntryData() {
  ...
  try {
    ...
    if (dynamicTagsData && Array.isArray(dynamicTagsData.tags)) {
      // Нормализуем теги критика: приводим к нижнему регистру и убираем дубликаты
  ...
  // Нормализуем статические теги из предыдущих советов
  const staticInspirationTags = extractTags(previousSuggestions).map(tag => tag.toLowerCase());
  const essayData = {
  ...
  // Нормализуем статические теги из новой записи
  const staticTags = extractTags(fullEntryClean).map(tag => tag.toLowerCase());
  console.log(`🏷️ Извлечено статических тегов: ${staticTags.length}`, staticTags);

  // Объединяем все теги с удалением дубликатов
...
async function updateAndSaveTagStatistics(currentStats, staticTags, criticTags, entryDate) {
  ...
  // Нормализуем все теги перед обработкой
  ...
  for (const tag of allTagsFromEntry) {
    if (updatedStats[tag]) {
      ...
      if (normalizedStaticTags.includes(tag) && !updatedStats[tag].types.includes('static')) updatedStats[tag].types.push('static');
      if (normalizedCriticTags.includes(tag) && !updatedStats[tag].types.includes('critic')) updatedStats[tag].types.push('critic');
    } else {
      updatedStats[tag] = {
        count: 1,
        firstSeen: entryDate,
