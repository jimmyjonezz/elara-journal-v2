...
function extractSimpleDynamicTags(text) {
  // --- Добавлено для диагностики кодировки ---
  if (text.length > 0) {
    console.log("🔍 [Динамические теги] Диагностика кодировки. Первые 50 символов текста:");
    const sample = text.substring(0, 50);
    const codes = [];
    for (let i = 0; i < sample.length; i++) {
      codes.push(sample.charCodeAt(i));
    }
    console.log("🔍 [Динамические теги] Символы:", JSON.stringify(sample));
    console.log("🔍 [Динамические теги] Их Unicode коды:", codes.join(', '));
    
    // Проверим конкретно буквы "С" и "е" из "Сегодня"
    if (text.length >= 2) {
      const firstChar = text[0];
      const secondChar = text[1];
      console.log(`🔍 [Динамические теги] Первый символ: '${firstChar}' (код: ${firstChar.charCodeAt(0)})`);
      console.log(`🔍 [Динамические теги] Второй символ: '${secondChar}' (код: ${secondChar.charCodeAt(0)})`);
      console.log("🔍 [Динамические теги] Код 'С' (русская заглавная):", 'С'.charCodeAt(0));
      console.log("🔍 [Динамические теги] Код 'е' (русская строчная):", 'е'.charCodeAt(0));
    }
  } else {
     console.log("🔍 [Динамические теги] Входной текст пуст.");
  }
  // --- Конец диагностики ---
  ...
  // --- Попробуем более "перmissive" регулярное выражение ---
  // const words = text.toLowerCase().match(/\b[а-яёa-z]+\b/g) || []; // Старое
  
  // Новое: Просто ищем последовательности кириллических и латинских букв
  ...
  if (words.length > 0) {
    ...
    console.log("🔤 [Динамические теги] ⚠️ НИ ОДНО СЛОВО НЕ ИЗВЛЕЧЕНО! Проверьте регулярное выражение и кодировку.");
  }
  ...
  const wordsToCheck = ['звук', 'звуки', 'сегодня', 'окно', 'писать', 'письмо', 'мои', 'мой', 'я', 'это', 'солнце', 'ajanely', 'балкон', 'кот', 'лайма', 'город', 'просыпался', 'луч', 'тишина', 'мысли', 'жизнь', 'река', 'берегу'];
