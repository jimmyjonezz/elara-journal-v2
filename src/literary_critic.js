...
async function saveDynamicTags(tags) {
  try {
    ...
    console.error(`❌ Ошибка при сохранении динамических тегов в ${DYNAMIC_TAGS_PATH}:`, error.message);
...
async function analyzeLatestEntry() {
  try {
    ...
    if (analysisResult && Array.isArray(analysisResult.tags_for_search)) {
       ...
       console.log(`🏷️ Извлечено тегов для поиска: ${dynamicTagsForSaving.length}`, dynamicTagsForSaving);
    } else {
