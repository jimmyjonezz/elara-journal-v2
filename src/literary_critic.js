...
// --- Добавлено: Путь к файлу динамических тегов ---
...
function formatEntryForContext(entry) {
  // Используем поле 'date' для отображения даты
...
async function createAnalysisPrompt(entry, historyContext) {
  console.log('Создание промпта для анализа записи:', entry.date); // Используем .date для логирования
...
// --- Добавлено: Функция для сохранения динамических тегов ---
/**
 * Сохраняет теги, сгенерированные критиком, в отдельный файл.
 * @param {Array<string>} tags - Массив тегов.
 */
async function saveDynamicTags(tags) {
  // --- Добавлено: Лог для отладки ---
  console.log("🔍 [DEBUG] saveDynamicTags вызвана с аргументом:", JSON.stringify(tags));
  // --- Конец добавления ---
  
  try {
    const tagData = {
      last_updated: new Date().toISOString().split('T')[0],
      tags: tags || [] // <-- Проверьте, что tags - это массив
    };
    
    // Убеждаемся, что директория существует
    const dirname = path.dirname(DYNAMIC_TAGS_PATH);
    try {
    ...
    await fs.writeFile(DYNAMIC_TAGS_PATH, JSON.stringify(tagData, null, 2));
    console.log(`✅ Динамические теги сохранены в ${DYNAMIC_TAGS_PATH}`);
  } catch (error) {
    // --- Изменено: Более явное сообщение об ошибке ---
    console.error(`❌ [КРИТИЧНО] Ошибка при сохранении динамических тегов в ${DYNAMIC_TAGS_PATH}:`, error.message);
    console.error("❌ [КРИТИЧНО] Стек ошибки:", error.stack);
    // --- Конец изменения ---
    // Не останавливаем выполнение из-за ошибки сохранения тегов
  }
}
// --- Конец добавления ---
...
async function analyzeLatestEntry() {
  try {
    ...
    // Берем последнюю запись (используем поле 'date')
    const latestEntry = journal[journal.length - 1];
    console.log(`Анализ записи от ${latestEntry.date}...`); // Используем .date для логирования
    
    // Создаем контекст из истории (используем поле 'date')
    ...
    try {
      ...
      try {
        ...
        console.error('Ошибка парсинга JSON после очистки Markdown:', secondParseError.message);
        ...
        if (jsonMatch) {
          try {
            ...
            console.error('Ошибка парсинга JSON регулярным выражением:', thirdParseError.message);
            // Если все методы парсинга неудачны, сохраняем "сырой" ответ
            console.log("⚠️ Не удалось распарсить JSON. Сохраняю 'сырой' ответ для диагностики.");
            
            const errorResult = {
              generated_at: new Date().toISOString().split('T')[0],
              error: `Ошибка парсинга JSON от LLM. Все методы неудачны. Ошибки: 1) ${parseError.message}, 2) ${secondParseError.message}, 3) ${thirdParseError.message}`,
              raw_response: response // Сохраняем полный ответ
            };
            
            await fs.writeFile(ANALYSIS_PATH, JSON.stringify(errorResult, null, 2));
            console.log(`⚠️ Анализ завершен с ошибкой. "Сырой" ответ сохранен в ${ANALYSIS_PATH}.`);
            return; // Успешно завершаем функцию, не прерывая процесс
          }
        } else {
            console.log("⚠️ JSON-объект не найден регулярным выражением. Сохраняю 'сырой' ответ для диагностики.");
            
            const errorResult = {
              generated_at: new Date().toISOString().split('T')[0],
              error: `Ошибка парсинга JSON от LLM. JSON-объект не найден. Ошибки: 1) ${parseError.message}, 2) ${secondParseError.message}`,
              raw_response: response // Сохраняем полный ответ
            };
    ...
    // --- Добавлено: Извлечение тегов и их сохранение ---
    // --- Добавлено: Лог для отладки ---
    console.log("🔍 [DEBUG] Подготовка к извлечению и сохранению тегов критика...");
    // --- Конец добавления ---
    ...
    // Если мы дошли до этого места, значит analysisResult успешно получен
    ...
    // Этот блок сработает на критические ошибки (например, проблемы с файлами, сетью, callOpenRouter)
    console.error('❌ Критическая ошибка в процессе анализа:', error);
    
    // Записываем критическую ошибку в файл анализа для диагностики
    const criticalErrorResult = {
      generated_at: new Date().toISOString().split('T')[0],
      error: `Критическая ошибка: ${error.message}`,
      stack: error.stack // Сохраняем стек вызовов для отладки
    };
    
    try {
      await fs.writeFile(ANALYSIS_PATH, JSON.stringify(criticalErrorResult, null, 2));
    ...
    // Вместо process.exit(1), завершаем функцию успешно, 
    // чтобы workflow не прерывался и продолжал работать
    return;
  }
}
